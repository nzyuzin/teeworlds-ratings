...
type t = {id: int64; name: string; description: string; sub_clan_id: int64}

let empty () = {id = Int64.minus_one; name = ""; description = ""; sub_clan_id = Int64.minus_one}
...
let of_row_e named_row additional_col =
  ...
  | ("description", Sqlite3.Data.TEXT description) -> {clan with description = description}
...
let select_stmt = "select id, name, description, sub_clan_id from clans where id = ?"

let select clan_id =
  let s = prepare_bind_stmt select_stmt [Sqlite3.Data.INT clan_id] in
  Option.map of_row (exec_select_single_row_stmt s)

let select_by_name_stmt = "select id, name, description, sub_clan_id from clans where name = ?"
