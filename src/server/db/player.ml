...
type t = {id: int64; name: string; clan: string; rating: int64; secret_key: string}

let empty () = {id = Int64.minus_one; name = ""; clan = ""; rating = Int64.minus_one; secret_key = ""}
...
let insert_stmt = "insert into players (name, clan, rating, secret_key) values (?, ?, ?, ?)"
...
let select_stmt = "select name, clan, rating from players where name = ?"
...
let select_by_id_stmt = "select name, clan, rating from players where id = ?"
...
let select_with_secret_stmt = "select name, clan, rating, secret_key from players where name = ?"
...
let select_by_rating_stmt = "select name, clan, rating from players order by rating DESC limit(?) offset(?)"
...
let select_by_clan_stmt = "select name, clan, rating from players where clan = ?"

let select_by_clan clan_name =
  let s = prepare_bind_stmt select_by_clan_stmt [Sqlite3.Data.TEXT clan_name] in
  of_rows (exec_select_stmt s)

let select_top5_stmt = "select name, clan, rating from players order by rating DESC limit(5)"
